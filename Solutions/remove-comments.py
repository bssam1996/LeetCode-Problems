    ...
    for line in source:
        if multiline_comment:
            if "*/" in line:
                ...
                continue
        if "//" in line:
            if "/*" not in line or ("/*" in line and line.index("/*") > line.index("//")):
                line = line[0:line.index("//")]
                if line != "":
                    output.append(line)
                continue
        if "/*" in line:
            temp_line = line[line.index("/*") + 2:]
            if "*/" in temp_line:
                temp_line = line[0:line.index("/*")]
                line = line[line.index("/*") + 2:]
                line = temp_line + line[line.index("*/")+2:]
                if "//" in line:
                    line = line[0:line.index("//")]
                if line != "":
                    if len(output) > 0:
                        if output[-1][-1] == ";" or output[-1][-1] == "{" or output[-1] == "blank":
                            output.append(line)
                        else:
                            output[-1] = output[-1] + line
                    else:
...
def testcases():
    list_of_cases = [
        ...
        # },
        # {
        #     "s":["class test{", "public: ", "   int x = 1;", "   /*double y = 1;*/", "   char c;", "};"],
        #     "o":["class test{","public: ","   int x = 1;","   ","   char c;","};"]
        # },
        # {
        #     "s":["struct Node{", "    /*/ declare members;/**/", "    int size;", "    /**/int val;", "};"],
        #     "o":["struct Node{","    ","    int size;","    int val;","};"]
        # },
        # {
        #     "s":["void func(int k) {", "// this function does nothing /*", "   k = k*2/4;", "   k = k/2;*/", "}"],
        #     "o":["void func(int k) {","   k = k*2/4;","   k = k/2;*/","}"]
        # },
        # {
        #     "s":["a//*b//*c","blank","d/*/e*//f"],
        #     "o":["a","blank","d/f"]
